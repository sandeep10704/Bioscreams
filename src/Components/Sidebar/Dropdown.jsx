...
const Dropdown = ({ title, icon: Icon, items, isOpen, toggleOpen }) => {
  return (
    <div>
      <button
        onClick={toggleOpen}
      ...
      >
        <span className="p-2 rounded-full text-gray-400">
          ...
          {items.map(({ name, path }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                ...
                ${isActive ? 'bg-purple-100 text-purple-700 font-semibold' : 'text-gray-600 hover:bg-gray-100'}`
              }
            >
              {name}
            </NavLink>
